...
struct Cli {
    ...
    /// store's place, so a restart resumes every one of them. Required
    /// with --select unless --dry-run
    #[arg(long, value_name = "PATH", requires = "select", help_heading = HEAD_POS)]
    positions: Option<PathBuf>,
    /// How long to wait before asking a selection for more, when the last
    /// answer was exhausted
    #[arg(long, value_name = "DUR", default_value = "1s", requires = "select", help_heading = HEAD_POS)]
    poll: String,
...
fn encoding_of(cli: &Cli) -> otlp::Encoding {
    if cli.encoding == "json" {
        otlp::Encoding::Json
    } else {
        otlp::Encoding::Proto
    }
...
fn resource_attrs(store: Option<&Path>, cli: &Cli) -> anyhow::Result<StoreIdentity> {
    let Some(path) = store else {
        return Ok(StoreIdentity {
            attrs: finish_resource(Vec::new(), cli.service.clone(), cli)?,
            anchor: None,
        });
    };
    let (dir, name) = timberfs::query::resolve_backing(path)?;
    let bark = timberfs::bark::load(&dir, &name);
    let anchor = Some(timberfs::cursor::store_anchor(&dir, &name, bark.as_ref()));
    let declared = bark.clone().unwrap_or_default();
    let id = declared
        .get("id")
        .and_then(|v| v.as_str())
        .map(str::to_string);
    Ok(StoreIdentity {
        attrs: store_resource(&name, id.as_deref(), path, &declared, cli)?,
        anchor,
    })
}

/// The resource of ONE store, from what it declares. Shared by the
/// single-store path and by every slice of a selection, so a set cannot
/// come to be labelled by a different rule than a single store is.
///
/// `declared` may be the whole manifest or just its provenance: only
/// declared keys are read, and `name`/`id` are passed separately because
/// the label set deliberately excludes them.
fn store_resource(
    name: &str,
    id: Option<&str>,
    path: &Path,
    ...
    let get = |k: &str| -> Option<String> {
        declared.get(k).and_then(|v| v.as_str()).map(str::to_string)
    };
    ...
    finish_resource(attrs, service, cli)
}

/// What every resource ends with, whatever it started from: a
/// `service.name`, a `host.name`, and the operator's own overrides last.
fn finish_resource(
...
impl Sender {
    ...
    fn flush(&mut self, batch: &mut Vec<EntryRec>) -> anyhow::Result<()> {
        ...
        let sent = self.deliver(&[otlp::Group {
            resource: &resource,
            entries: &entries,
        }]);
    ...
    /// One request, however many stores it carries. A retryable endpoint
    /// is retried forever with capped backoff — the store holds the
    /// backlog, and giving up would lose data an endpoint that comes back
    /// could have taken; a permanent refusal drops the batch loudly and
    /// moves on, because the alternative is a stream that never advances
    /// again.
    fn deliver(&mut self, groups: &[otlp::Group]) -> anyhow::Result<()> {
        let count: usize = groups.iter().map(|g| g.entries.len()).sum();
        if count == 0 {
            return Ok(());
        }
        if self.dry_run {
                ...
                match client.post(&text) {
                    Outcome::Delivered { rejected, message } => {
                        if rejected > 0 {
                            ...
                            eprintln!(
                                "timber-otlp: {rejected} of {count} record(s) refused by the \
                                 receiver{}",
                    ...
                    Outcome::Rejected(why) => {
                        ...
                        eprintln!(
                            "timber-otlp: {count} record(s) permanently refused ({why}); \
...
fn run() -> anyhow::Result<()> {
    ...
    if let Some(expr) = cli.select.clone() {
        return run_selection(&cli, &expr);
    }
    ...
    let encoding = encoding_of(&cli);
    let client = client_of(&cli, encoding, timeout)?;
    ...
    loop {
        match rx.recv_timeout(batch_timeout) {
            Ok(Msg::Entry(e)) => {
                if sender.cursor.is_some() && (e.wf.is_none() || e.wl.is_none()) {
                    bail!(
            ...
            Err(RecvTimeoutError::Timeout) => sender.flush(&mut batch)?,
    ...
    note!(
        ...
        sender.shipped,
        if sender.shipped == 1 { "y" } else { "ies" },
        sender.batches,
        if resume.skipped() > 0 {
            format!(
        ...
        if sender.rejected > 0 {
            format!("; {} refused", sender.rejected)
        } else {
            String::new()
        }
    );
...
/// Ship a SELECTION: one process, one request per batch, one position per
/// store.
///
/// The loop is a poll and not a stream, because a resumable position for
/// a SET is what a bounded read answers with — one `position` record per
/// store — where a followed stream carries one start for every source at
/// once. A resumed bounded read includes the write-ahead edge, so nothing
/// is traded for it.
fn run_selection(cli: &Cli, expr: &str) -> anyhow::Result<()> {
    let selector = timberfs::select::Selector::parse(expr)?;
    if cli.positions.is_none() && !cli.dry_run {
        // Without a file the loop advances in memory and every restart
        // re-ships every store from the start, which is the one failure
        // this whole mechanism exists to prevent.
        bail!(
            "--select needs --positions PATH: a selection's position is one per store, and \
             without somewhere to keep them a restart re-ships every one of them"
        );
    }
    let poll = Duration::from_millis(parse_duration_ms(&cli.poll)?.max(1));
    let timeout = Duration::from_millis(parse_duration_ms(&cli.timeout)?.max(1));
    let severity = Severity::new(cli.severity_regex.as_deref())?;
    let encoding = encoding_of(cli);
    let mut sender = Sender {
        client: client_of(cli, encoding, timeout)?,
        encoding,
        resource: Vec::new(),
        severity,
        cursor: None,
    ...
    let mut shipper =
        timberfs::ship::Shipper::open(CONSUMER, selector, Vec::new(), cli.positions.as_deref())?
            .with_batch_entries(cli.batch_size as u64);
    if cli.dry_run {
        shipper = shipper.read_only();
    }
    let mut said: Option<usize> = None;
    loop {
        let batch = shipper.poll()?;
        // How many stores the selector matches is the one number an
        // operator needs and cannot infer: nothing arriving is the same
        // observation whether it matched fifty quiet stores or none.
        if said != Some(batch.matched) {
            note!("timber-otlp: {} following {} store(s)", expr, batch.matched);
            said = Some(batch.matched);
        }
        if !batch.is_empty() {
            let resources = batch
                .slices
                .iter()
                .map(|s| store_resource(&store_name(&s.path), Some(&s.id), &s.path, &s.labels, cli))
                .collect::<anyhow::Result<Vec<_>>>()?;
            let entries: Vec<Vec<Entry>> = batch
                .slices
                .iter()
                .map(|s| {
                    s.entries
                ...
                .collect();
            let groups: Vec<otlp::Group> = resources
                .iter()
                .zip(&entries)
                ...
                .collect();
            sender.deliver(&groups)?;
            // Only now: the receiver has it, which is the only proof a
            // position may advance on.
            shipper.accept(&batch)?;
        }
        if batch.more {
            continue;
        }
...
/// A store's name as the resource wants it: its file name without the
/// `.log` a log store carries.
