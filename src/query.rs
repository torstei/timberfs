...
/// The window CHUNK selection runs on: what was asked, widened so a line
/// written a little before or after the stamp it carries is still read.
pub(crate) fn widened(from_ms: u64, to_ms: u64) -> (u64, u64) {
    (
...
fn query_entries<W: Write>(
    ...
    for f in files {
        ...
        let (widened_from, widened_to) = widened(from_ms, to_ms);
