...
where to begin and no way to re-ship a store by getting one wrong. ⚠ But it
cannot hold retention back yet: `retain_unconsumed` reads the follower
registry, and frames want the `chunks` diet a follower does not serve, so a
frames edge store is bounded by `retain_size` alone.
